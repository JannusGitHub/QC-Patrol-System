...
<table style="font-family: Arial; width: 100%;" border="1">
	<thead>
		...
		<tr>
			...
			<th colspan="3" style="width: 40%; text-align: left;"><b>{{ \Carbon\Carbon::parse($data->audit_date)->isoFormat('ll') }}</b></th>
			<th colspan="4" style="width: 20%; text-align: left;"><b>Issued Date: {{ \Carbon\Carbon::parse($data->issued_date)->isoFormat('ll') }}</b></th>
		...
		<tr>
			...
			<th colspan="3" style="width: 40%; text-align: left;"><b>{{ $department }}</b></th>
			<th colspan="4" style="width: 20%; text-align: left;"><b>Due Date: {{ \Carbon\Carbon::parse($data->due_date)->isoFormat('ll') }}</b></th>
			<!-- <th colspan="2" style="width: 30%; text-align: left;"></th> -->
		</tr>
		<tr>
			@php
			...
			<th colspan="3" style="width: 40%; text-align: left;"><b>{{ $auditor }}</b></th>
			<th colspan="4"></th>
		</tr>
		<tr>
			@php
			...
			<th colspan="3" style="width: 40%; text-align: left;"><b>{{ $auditee }} {{ $auditee_manual }}</b></th>
		...
		<tr>
			<th colspan="8" style="width: 50%; height: 70px;">{!! nl2br(e($data->commendation)) !!}</th>
		</tr>
		...
		<tr>
			...
			<th style="background-color: #cc9ccc; border: 1px solid black;"><b>Close-out Audit Result 
				<!-- <br /> {{ \Carbon\Carbon::parse($data->due_date)->isoFormat('ll') }} -->
			</b></th>
		</tr>
	</thead>
	<tbody>
		@for($index = 0; $index < count($data->ar_finding_details); $index++)
			<tr>
				<th style="border: 1px solid black;"><center>{{ $data->ar_finding_details[$index]->no }}</center></th>
				<th style="border: 1px solid black;">{{ $data->ar_finding_details[$index]->station }} / <br /> {{ $data->ar_finding_details[$index]->classification_info->name }}</th>
				<th style="border: 1px solid black;">
                	{{ explode(' ', $data->ar_finding_details[$index]->factor_item_list_info->name)[0] }} {{ explode(' ', $data->ar_finding_details[$index]->factor_item_list_info->name)[1] }} 
                	<b>({{ $data->ar_finding_details[$index]->factor_info->name }})</b><br>
					{!! nl2br(e($data->ar_finding_details[$index]->statement_of_findings)) !!}
				</th>
				...
				<td style="border: 1px solid black;">
					{{-- {{ $result }} --}}
					{{-- {{ $data->ar_finding_details[$index]->actual_illustration }} <br> --}}
				...
				<th style="border: 1px solid black;">{{ $in_charge }}</th>
				<th style="border: 1px solid black;">{{ $data->ar_finding_details[$index]->counter_measures }}</th>
				<th style="border: 1px solid black;">{{ $data->ar_finding_details[$index]->commitment_date }}</th>
				@php
				...
				<th style="border: 1px solid black;">
					{{ $data->ar_finding_details[$index]->close_out }}
					<!-- <center><img src="{{ $path }}" style="max-width: 100px; max-height: 100px; min-width: 20px; min-height: 20px;" class="commonAuditImg"></center> -->
				</th>
			</tr>
		@endfor
	</tbody>
