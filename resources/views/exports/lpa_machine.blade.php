	...
	<tbody>
		...
		<tr>
			...
			@if($data->ar_finding_details != null){
				@for($index = 0; $index < count($factor_item_list_machine); $index++){
					<tr>
						<td	d colspan="10" style="border: 1px solid black; word-wrap: break-word;">{{ $factor_item_list_machine[$index]->name }}</td>
							@php
								$isChecked = '√';
								$referToQCPatrol = '';
							...
							@for($j = 0; $j < count($data->ar_finding_details); $j++)
								@if($data->ar_finding_details[$j]->factor_item_list_id == $factor_item_list_machine[$index]->id)
									@php
										$isChecked = 'X';
							...
							@if($factor_item_list_machine[$index]->id == '92')
								@php
									$isChecked = '';
									$referToQCPatrol = '';
									$bgColor = 'background-color: none';
