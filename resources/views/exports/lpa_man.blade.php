	...
	<tbody>
		...
		<tr>
			...
			@if($data->ar_finding_details != null){
				@for($index = 0; $index < count($factor_item_list); $index++){
					<tr>
						<td	d colspan="10" style="border: 1px solid black; word-wrap: break-word;">{{ $factor_item_list[$index]->name }}</td>
							@php
								$isChecked = '√';
								$referToQCPatrol = '';
							...
							@for($j = 0; $j < count($data->ar_finding_details); $j++)
								@if($data->ar_finding_details[$j]->factor_item_list_id == $factor_item_list[$index]->id)
									@php
										$isChecked = 'X';
							...
							@if($factor_item_list[$index]->id == '92' || $factor_item_list[$index]->id == '65' || $factor_item_list[$index]->id == '68' || $factor_item_list[$index]->id == '73')
								@php
									$isChecked = '';
									$referToQCPatrol = '';
									$bgColor = 'background-color: none';
