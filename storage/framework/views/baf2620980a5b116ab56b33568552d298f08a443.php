	...
	<tbody>
		...
		<tr>
			...
			<?php if($data->ar_finding_details != null): ?>{
				<?php for($index = 0; $index < count($factor_item_list_material); $index++): ?>{
					<tr>
						<td	d colspan="10" style="border: 1px solid black; word-wrap: break-word;"><?php echo e($factor_item_list_material[$index]->name); ?></td>
							<?php
								$isChecked = '√';
								$referToQCPatrol = '';
							...
							<?php for($j = 0; $j < count($data->ar_finding_details); $j++): ?>
								<?php if($data->ar_finding_details[$j]->factor_item_list_id == $factor_item_list_material[$index]->id): ?>
									<?php
										$isChecked = 'X';
							...
							<?php if($factor_item_list_material[$index]->id == '92'): ?>
								<?php
									$isChecked = '';
									$referToQCPatrol = '';
									$bgColor = 'background-color: none';
...
</table><?php /**PATH /var/www/qc_patrol_new/resources/views/exports/lpa_material.blade.php ENDPATH**/ ?>