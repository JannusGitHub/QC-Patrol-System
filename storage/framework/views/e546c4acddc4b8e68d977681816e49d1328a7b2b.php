...
<table style="font-family: Arial; width: 100%;" border="1">
	...
	<tbody>
		<tr>
			<th colspan="10" style="border: 1px solid black"><b>FACTOR: METHOD</b></th>
		...
		<tr>
			...
			<?php if($data->ar_finding_details != null): ?>{
				<?php for($index = 0; $index < count($factor_item_list_method); $index++): ?>{
					<tr>
						<td	d colspan="10" style="border: 1px solid black; word-wrap: break-word;"><?php echo e($factor_item_list_method[$index]->name); ?></td>
							<?php
								$isChecked = '√';
								$referToQCPatrol = '';
							...
							<?php for($j = 0; $j < count($data->ar_finding_details); $j++): ?>
								<?php if($data->ar_finding_details[$j]->factor_item_list_id == $factor_item_list_method[$index]->id): ?>
									<?php
										$isChecked = 'X';
							...
							<?php if($factor_item_list_method[$index]->id == '75'): ?>
								<?php
									$isChecked = '';
									$referToQCPatrol = '';
									$bgColor = 'background-color: none';
...
</table><?php /**PATH /var/www/qc_patrol_new/resources/views/exports/lpa_method.blade.php ENDPATH**/ ?>