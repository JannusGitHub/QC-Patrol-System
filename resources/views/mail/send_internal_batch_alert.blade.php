					...
					@for($index = 0; $index < count($internals); $index++)
						@if($internals[$index]->audit_stat == 2 && $internals[$index]->audit_type == 'Process')
							<tr>
								...
								<td>
				                    @for($index2 = 0; $index2 < count($internals[$index]->person_in_charges); $index2++)
				                        @if(count($internals[$index]->person_in_charges) > 0)
				                            @isset($internals[$index]->person_in_charges[$index2]->person_in_charge_record->name)
				                                {{ $internals[$index]->person_in_charges[$index2]->person_in_charge_record->name }}
				                            @endisset
				                            @if($index2 <= (count($internals[$index]->person_in_charges) - 2))
				                                , 
				                            @endif
				                        @endif
				                    @endfor
